...
st.markdown(
    """
Upload:
1. **GEO2R full results table** (Excel/CSV)  
2. **GPL platform table** (Excel/CSV, with `ID` and `ORF` columns)  

The app will:
- Merge on **ID**
- Attach **Locus_tag** from `ORF`
- Apply **adj.P.Val** and **logFC** thresholds
- Give you **Test-up** and **Control-up** gene lists (SO_XXXX)
"""
)
...
logfc_cutoff = st.sidebar.number_input(
    ...
    min_value=0.0, max_value=10.0, value=1.0, step=0.25
)

add_underscore = st.sidebar.checkbox(
    "Convert SOxxxx → SO_xxxx", value=True,
    help="If checked, ORF like 'SO1427' becomes 'SO_1427'."
)

one_probe_per_gene = st.sidebar.checkbox(
    "Keep one probe per gene (max |logFC|)", value=True
)
...
def read_uploaded_file(uploaded_file):
    ...
    if filename.endswith(".xls") or filename.endswith(".xlsx"):
        ...
        # try CSV/TSV with autodetect separator
...
merged.rename(columns={"ORF": "Locus_tag"}, inplace=True)

# Drop rows without locus tag
...
# Optional: add underscore to SO ids
if add_underscore:
    def fix_so(tag):
        tag = str(tag)
...
merged_deg = merged[
    (merged["adj.P.Val"] <= fdr_cutoff) &
    (merged["logFC"].abs() >= logfc_cutoff)
].copy()
...
# Split into test-up (logFC >= +cutoff) and control-up (logFC <= -cutoff)
...
def series_to_txt_bytes(series):
    buf = io.StringIO()
...
st.markdown(
    """
Now you can:
- Use the **locus tag lists** directly in **Venny** and **ShinyGO/KEGG**  
- Use the **DEG table** for your thesis tables and interpretation
"""
)
